...
import widgets.AdvancedRecurrenceWidget;
import widgets.FormatBox;
import widgets.NthDayOfMonthRecurrenceWidget;
import widgets.NthDayRecurrenceWidget;
import widgets.RecurrenceWidget;
import widgets.WeeklyRecurrenceWidget;
...
public abstract class EventDialog implements ActionListener, Observer {
	...
	public void fillIn(Event e){
		...
		startTime.setText((hour > 12 ? hour - 12: hour) + ":" + (min == 0 ? "00" : min)+
				(hour >= 12 ? "PM": "AM"));

		hour = e.getPeriod().end.time.hour;
		min = e.getPeriod().end.time.minutes;
		endTime.setText((hour > 12 ? hour - 12: hour) + ":" + (min == 0 ? "00" : min) +
				(hour >= 12 ? "PM": "AM"));
	...
	protected Period validateTime() throws Exception{
		if ( startDate.isValid() && endDate.isValid() ) {
			...
			case 1:
				//the time is one large string
				if(startTT[0].length() < 3){
					//must have at least 3 digits(one for the hour,
					//two for the minutes
				...
				break;
			case 2:
				//hour and minute are seperated by a colon
			...
			if( startT.contains("p") || startT.contains("P")){
				//assume that this is PM
				startHour += 12; //since this is PM, must add 12 to get to the proper military time
			...
			return Period.parse(startDate.format() + ":" + startHour + "." + startMin + "-" +
					endDate.format() + ":" + endHour + "." + endMin);
		}else{
