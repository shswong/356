...
{
        private ArrayList<DatabaseEvent> listOfEvents; 
        ...
        public taskManager() throws Exception {
                listOfEvents = new ArrayList<DatabaseEvent>();
                globalClassNumber = 0;//used to give new class events a unique classNumber
                db = new DatabaseConnection();
                db.countClasses();
        }
      //5/31/2010 10:00:00
		public DatabaseEvent convertToDBEvent(Event e)
		{
			DatabaseEvent dbe = new DatabaseEvent();
			
			String startAM, endAM;
			
			
			Period p = e.getPeriod();
			
			if ( p.start.time.hour < 12 )
				startAM = "AM";
			else startAM = "PM";
			
			if ( p.end.time.hour < 12 )
				endAM = "AM";
			else endAM = "PM";
			
			dbe = new DatabaseEvent(e.getDescription(),
					dbe.getTimeArr(p.start.date.month + "/" + p.start.date.day + "/" + p.start.date.year + " " + p.start.time.hour + ":" + p.start.time.minutes + ":00 " + startAM ),
					dbe.getTimeArr(p.end.date.month + "/" + p.end.date.day + "/" + p.end.date.year + " " + p.end.time.hour + ":" + p.end.time.minutes + ":00 " + endAM ),e.getTitle(),0,1);
        ...
        public boolean addEvent(DatabaseEvent e) throws Exception {
        ...
        public void absoluteAdd(DatabaseEvent e) throws Exception
        {
        	db.add(e);
        }
        ...
        public void addStudyEvent(String eventName, String startTime, String endTime, String eventType, int classNumber, int priority) throws Exception {
                ...
                db.add(new DatabaseEvent(eventName, startDate, endDate,eventType,classNumber,priority));
        ...
        public void edit(int ID, DatabaseEvent e) throws Exception {
                for (int i = 0; i < listOfEvents.size(); i++)
                {
